...
def detect_image(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
    custom_image_bgr = im#cv2.imread(im) 
    ...
    if showImage:
        try:
            ...
            for detection in detections:
                ...
                image = cv2.rectangle(image, (xCoord, yCoord),  (xCoord + xEntent, yCoord + yExtent), boxColor,thickness=2) 
...
def pro(frame):
    ...
    img=frame.copy()
    performDetect(img, thresh= 0.1,  configPath = "./cfg/yolov3-tiny.cfg", weightPath = "yolov3-tiny.weights", metaPath= "./cfg/coco.data", showImage= True, makeImageOnly = False, initOnly= False)
    return img

import Camera

pip=Camera.Pipelines(func=pro)
pip.startStream()